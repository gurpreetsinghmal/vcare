...
class ApiController extends Controller
{
    ...
    public function gdmoupdatepatient(Request $request)
    {
        $token = trim($request["access_token"]);
        $gdmo = User::where('access_token', $token)
            ->select(
                'id',
                'name',
                'mobile',
        ...
        if ($gdmo && $gdmo->role_id == 3) {
            ...
            if ($u) {
                ...
                if ($request["section"] == "gdmoprepreg") {

                    $u->refergyno = $request["refergyno"];
                    $u->siteofdelivery = $request["siteofdelivery"];
                    $u->mcp = $request["mcp"];
                }

                if ($request["section"] == "gdmopostpreg") {

                    $u->modeofdelivery = $request["modeofdelivery"];
                    $u->dateofdelivery = $request["dateofdelivery"];
                    $u->sexofchild = $request["sexofchild"];
                }
